...
def _compose_github_digest(
    ...
    try:
        structured_items = _summarize_items_for_github(
            items=items,
            openai_api_key=openai_api_key,
            openai_model=openai_model,
            azure_openai_endpoint=azure_openai_endpoint,
    ...
    return _compose_github_markdown(items, channel_label, today, structured_items)
...
def _compose_github_markdown(
    ...
    for index, item in enumerate(items, start=1):
        ...
        core = structured.get("core") or summary or "요약 정보 없음"
        business_angle = (
            structured.get("business_angle")
            or "이 흐름을 실제 고객 문제, 반복 업무, 운영 효율, 또는 신규 수익 모델과 연결해 볼 수 있습니다."
        )
        mentoring_question = (
            structured.get("mentoring_question")
            or "이 흐름을 어떤 고객 문제나 수익 모델로 바꿀 수 있을까요?"
        )
        lines.extend(
            [
                f"## {index}. {title}",
...
def _summarize_items_for_github(
    items: list[DigestItem],
    openai_api_key: str | None,
    openai_model: str,
    azure_openai_endpoint: str | None,
    ...
    if len(parsed) != len(items):
        raise ValueError("OpenAI returned a different number of items.")
    return parsed
