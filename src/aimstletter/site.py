...
def _items_archive_search_text(items: list[SiteItem]) -> str:
    parts: list[str] = []
    for item in items:
        parts.extend(
            [
...
def _archive_page_search_text(path: Path) -> str:
    try:
        html = path.read_text(encoding="utf-8")
    except OSError:
        return ""
    parts = re.findall(
        r'data-(?:title|category|subcategory|body|detail|meta|points|tags|criteria|footnotes)="([^"]*)"',
        html,
    )
    parts.extend(re.findall(r'<span class="detail-tag">#?([^<]*)</span>', html))
    return _normalize_search_text(" ".join(parts))
                ...
                search_text = _normalize_search_text(
                    " ".join(
                        [
                            ...
                            f"{year} {month:02d} {int(entry['week'])}",
                            str(entry.get("period_label", "")),
                        ]
                    )
                )
                links.append(
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    @media (max-width: 760px) {{
      ...
      .insight-detail h3 {{ font-size: 19px; }}
...
def _latest_week_specific_summary(text: str) -> tuple[str, tuple[str, str, str]] | None:
    ...
    rules: tuple[tuple[tuple[str, ...], str, tuple[str, str, str]], ...] = (
        ...
        (
            ...
            (
                "1. 무엇을 다루나요? 기존 DBMS 위에서 특정 질의를 빠르게 처리할 전용 가속기를 자동으로 만드는 JetStream입니다.",
                "2. 핵심 구성 요소: 측정 기반 생성 워크플로, 엔진 공통 실행 계층, DBMS별 어댑터, 상태 유지와 갱신 정책입니다.",
                "3. 업무 적용 포인트: 데이터베이스를 새로 바꾸기보다 반복 조회가 느린 지점을 먼저 측정하고, 기존 엔진과 연결 가능한 최적화부터 검토할 수 있습니다.",
            ),
        ),
        (
            ("2608.22063",),
            "이 논문은 AI가 매번 SQL을 작성하게 하기보다, 업무 의미에 맞춘 작은 도구를 고르게 하는 MCP 서버 설계를 제안합니다. 서버가 스키마 탐색과 조인, 업무 규칙을 처리하면 더 작은 모델도 고객 관련 데이터 요청을 더 정확하고 저렴하게 처리할 수 있음을 평가했습니다.",
            (
                "1. 무엇을 다루나요? SQL 생성 대신 업무별 도구를 선택하게 만드는 MCP 데이터 서버 설계입니다.",
                "2. 핵심 구성 요소: 도메인별 도구, 매개변수 SQL, 서버 측 스키마·업무 규칙, 의도 분류, MCP Blueprint입니다.",
                "3. 업무 적용 포인트: 데이터 에이전트에 광범위한 SQL 실행 권한을 주기보다, 자주 쓰는 업무를 안전한 도구로 잘게 나누는 방식이 유용합니다.",
            ),
        ),
        (
            ("2608.19025",),
            "이 연구는 LLM이 과학 논문에서 맥락 있는 데이터를 추출할 때, 모델이 스스로 만든 프롬프트와 여러 모델의 교차 검토를 함께 쓰는 방법을 평가합니다. 자동 문헌 탐색은 참고문헌 누락이나 허위 인용 문제가 있었고, 최종 판단에는 사람 검토가 필요하다는 점을 보여줍니다.",
            (
                "1. 무엇을 다루나요? 과학 논문에서 정보를 뽑을 때 자기 프롬프팅과 여러 모델의 합의를 활용하는 데이터 추출 연구입니다.",
                "2. 핵심 구성 요소: 전문가 프롬프트, 모델의 자체 프롬프트 작성, 반복 추출 교차 검토, 사람 검토 단계입니다.",
                "3. 업무 적용 포인트: 문서에서 중요한 데이터를 뽑을 때 AI 결과를 바로 확정하지 말고, 근거 기준과 의견이 갈린 항목을 사람이 확인하는 절차를 둬야 합니다.",
            ),
        ),
        (
            ("agnost ai",),
        ...
        (
            ...
            (
                "1. 무엇을 다루나요? 기업 AI 에이전트 운영에서 자격 증명 공유와 권한 관리가 만드는 보안 사고 위험입니다.",
                "2. 핵심 구성 요소: 에이전트 사고 경험, credential sharing, 권한 분리, 비밀값 관리, 감사 로그입니다.",
                "3. 업무 적용 포인트: 에이전트를 여러 업무에 붙일 때는 공용 계정과 토큰 공유를 줄이고 최소 권한과 추적 가능한 실행 이력을 먼저 설계해야 합니다.",
            ),
        ),
        (
            ("agent evaluation gap",),
            "엔터프라이즈 AI 조직이 평가 범위 부족보다 현실 업무와 평가 기준이 맞지 않는 문제를 겪고 있다는 분석입니다. 실제 운영에 넣기 전에는 데모 점수보다 업무 흐름, 실패 조건, 사람의 검수 기준과 맞는 평가가 필요합니다.",
            (
                "1. 무엇을 다루나요? 기업 AI 평가가 실제 업무 현실과 어긋나는 reality alignment 문제입니다.",
                "2. 핵심 구성 요소: 평가 기준, 운영 환경, 실패 조건, production 배포 리스크, 현실 정렬입니다.",
                "3. 업무 적용 포인트: AI 기능을 출시하기 전에는 벤치마크 점수보다 실제 업무 데이터와 예외 상황에서 검증되는지 봐야 합니다.",
            ),
        ),
        (
            ("agentic orchestration",),
            "엔터프라이즈 AI 조직이 챗봇을 에이전트라고 부르지만 실제 문제는 플랫폼 선택보다 배포 가능한 오케스트레이션 설계에 있다는 분석입니다. 상태, 도구 호출, 재시도, 승인 단계를 운영 가능한 실행 흐름으로 묶는 것이 핵심입니다.",
            (
                "1. 무엇을 다루나요? 에이전트 오케스트레이션을 실제 배포 가능한 업무 실행 구조로 만드는 문제입니다.",
                "2. 핵심 구성 요소: 상태 관리, 도구 호출, 승인 단계, 재시도, 운영 배포, 관측성입니다.",
                "3. 업무 적용 포인트: 챗봇형 응답을 에이전트라고 부르기보다 실행 상태와 실패 복구가 있는 워크플로로 설계해야 합니다.",
            ),
        ),
        (
            ("ai compute gap",),
            "기업들이 AI 인프라는 빠르게 구매하지만 실제 비용과 활용 효율을 충분히 측정하지 못한다는 분석입니다. GPU, 추론 비용, 사용량, 업무별 효과를 연결해 투자 대비 성과를 볼 수 있는 운영 지표가 필요합니다.",
            (
                "1. 무엇을 다루나요? AI 컴퓨트 인프라 구매 속도와 비용 측정 역량 사이의 격차입니다.",
                "2. 핵심 구성 요소: GPU·추론 비용, 사용량 측정, 비용 배분, 업무 효과, 인프라 거버넌스입니다.",
                "3. 업무 적용 포인트: AI 인프라를 늘릴 때는 용량 확보와 함께 팀·서비스별 비용 추적과 활용률 지표를 같이 만들어야 합니다.",
            ),
        ),
        (
            ("ai context gap",),
            "엔터프라이즈 AI에서 검색 품질보다 컨텍스트를 믿을 수 있는지, 어떤 근거를 사용했는지, 권한과 최신성이 맞는지가 더 큰 문제라는 분석입니다. RAG를 단순 검색이 아니라 신뢰 가능한 컨텍스트 공급 체계로 운영해야 한다는 내용입니다.",
            (
                "1. 무엇을 다루나요? 기업 AI에서 retrieval 자체보다 컨텍스트 신뢰와 권한 검증이 부족한 문제입니다.",
                "2. 핵심 구성 요소: RAG, 출처 신뢰도, 권한 필터링, 최신성, 컨텍스트 감사입니다.",
                "3. 업무 적용 포인트: 내부 지식 AI는 검색 성능뿐 아니라 누가 볼 수 있는 정보인지와 근거가 최신인지까지 검증해야 합니다.",
            ),
        ),
        (
            ("2607.12188",),
            "멀티테넌트 RAG에서 생성 비용은 토큰 단위로 측정되지만 검색 계층의 벡터 메모리, 유사도 계산, 임베딩 API 비용은 공유 비용으로 남는 문제를 다룬 연구입니다. 테넌트별 검색·생성 비용을 통합 관측해 보이지 않는 교차 보조를 줄이는 구조를 제안합니다.",
            (
                "1. 무엇을 다루나요? 멀티테넌트 RAG에서 검색 비용과 생성 비용을 테넌트별로 함께 귀속하는 Cost-Governed RAG입니다.",
                "2. 핵심 구성 요소: TurboVec, vector index, multi-tenant gateway, retrieval cost attribution, observability입니다.",
                "3. 업무 적용 포인트: 사내 RAG를 여러 팀이 함께 쓰면 토큰 비용뿐 아니라 벡터 검색과 임베딩 비용까지 사용자·테넌트별로 나눠 봐야 합니다.",
            ),
        ),
        (
            ("2607.11972",),
            "SkillComm은 무선 시각 지능이 단일 작업 추론을 넘어 순차적 스킬 워크플로로 이동할 때 필요한 의미 통신 구조입니다. 모든 토큰을 매번 보내기보다 스킬 상태와 실행 메모리를 재사용해 채널 제약 아래에서 필요한 정보를 점진적으로 전송합니다.",
            (
                "1. 무엇을 다루나요? 순차적 시각 AI 워크플로에서 스킬 중심으로 의미 정보를 전송하는 SkillComm입니다.",
                "2. 핵심 구성 요소: Skill-Compiler, Skill-Book, incremental token transmission, workflow memory, channel constraint입니다.",
                "3. 업무 적용 포인트: 엣지·무선 AI는 전체 데이터를 매번 보내기보다 업무 단계에 필요한 의미와 상태만 전달하도록 설계해야 합니다.",
            ),
        ),
        (
            ("2607.15263",),
            "공격·방어 보안 에이전트를 성공률만으로 평가하지 않고 추론 비용, 도구 호출 비용, 텔레메트리 조회 비용까지 함께 비교하는 연구입니다. Cybench와 Splunk BOTS 과제를 통해 red-team과 blue-team 작업의 비용 대비 성능 양상이 다름을 보여줍니다.",
            (
                "1. 무엇을 다루나요? 보안 에이전트의 공격·방어 성능을 비용 관점에서 평가하는 방식입니다.",
                "2. 핵심 구성 요소: Cybench, Splunk BOTS, inference spend, tool spend, cost-success tradeoff입니다.",
                "3. 업무 적용 포인트: SOC나 보안 자동화 에이전트는 최고 성공률보다 제한된 예산 안에서 어떤 모델과 도구 사용이 실용적인지 봐야 합니다.",
            ),
        ),
        (
            ("2607.15235",),
            "무선 센서 네트워크에서 배터리를 아끼기 위해 희소 샘플링을 하면서도 짧고 국소적인 이상 징후를 놓치지 않도록 sentinel-assisted adaptive sampling을 제안한 연구입니다. 정상 시기에는 예측 불확실성 기반으로 낮게 샘플링하고, 이상 시기에는 주변 노드가 협력해 샘플링 밀도를 높입니다.",
            (
                "1. 무엇을 다루나요? 무선 센서 네트워크에서 이상 징후를 놓치지 않으면서 샘플링 비용을 줄이는 적응형 방식입니다.",
                "2. 핵심 구성 요소: Kalman filter uncertainty, sentinel node, adaptive sampling, anomaly diffusion, cooperative sensing입니다.",
                "3. 업무 적용 포인트: IoT 모니터링은 평상시 비용 절감과 이상 상황의 빠른 감지를 동시에 만족하도록 샘플링 정책을 동적으로 바꿔야 합니다.",
            ),
        ),
        (
            ("2607.14798",),
            "클라우드 네이티브 위성 플랫폼에서 지상에서 작성한 미션 계획을 실행 가능한 워크플로 아티팩트로 컴파일하고 policy-as-code 가드레일로 검증하는 연구입니다. 위성 온보드 런타임 이전 단계의 지상 도구 체인 공백을 메우는 데 초점이 있습니다.",
            (
                "1. 무엇을 다루나요? 위성 미션 계획을 정책 검증된 실행 워크플로로 컴파일하는 Satellite Mission Compiler입니다.",
                "2. 핵심 구성 요소: mission plan compilation, policy-as-code, cloud-native satellite runtime, guardrail validation입니다.",
                "3. 업무 적용 포인트: 물리 인프라 자동화는 실행 전 계획 컴파일과 정책 검증 단계를 분리해 위험한 명령이 운영 환경에 들어가지 않게 해야 합니다.",
            ),
        ),
        (
            ("repository level github copilot usage metrics",),
            "GitHub Copilot 사용량 지표를 저장소 단위로 볼 수 있게 된 업데이트입니다. 조직은 어떤 저장소에서 Copilot이 많이 쓰이는지 파악해 도입 효과와 비용, 교육 대상을 더 세밀하게 관리할 수 있습니다.",
            (
                "1. 무엇을 다루나요? Copilot 사용량 메트릭을 repository level로 제공하는 GitHub 업데이트입니다.",
                "2. 핵심 구성 요소: 저장소별 사용량, 조직 리포팅, Copilot adoption, 비용·효과 분석입니다.",
                "3. 업무 적용 포인트: AI 코딩 도구 도입 효과는 조직 평균보다 저장소·팀별 사용 편차를 봐야 개선 지점을 찾을 수 있습니다.",
            ),
        ),
        (
            ("copilot app", "usage metrics api"),
            "GitHub Copilot app이 usage metrics API에서 제공되어 앱 단위 사용 현황을 자동으로 조회할 수 있게 된 업데이트입니다. 운영팀은 대시보드나 비용 관리 자동화에 Copilot 사용 데이터를 더 쉽게 연결할 수 있습니다.",
            (
                "1. 무엇을 다루나요? Copilot app 사용량 데이터를 usage metrics API에서 조회하는 기능입니다.",
                "2. 핵심 구성 요소: Usage Metrics API, Copilot app, 자동 리포팅, 사용량 대시보드입니다.",
                "3. 업무 적용 포인트: AI 도구 운영 데이터는 수동 보고보다 API로 수집해 비용·정책·교육 지표와 연결하는 편이 좋습니다.",
            ),
        ),
        (
            ("copilot code review customization",),
            "GitHub Copilot 코드 리뷰의 커스터마이징과 설정 가능성이 개선된 업데이트입니다. 팀은 리뷰 기준, 제안 방식, 저장소별 동작을 더 세밀하게 조정해 자동 리뷰를 팀 규칙에 맞출 수 있습니다.",
            (
                "1. 무엇을 다루나요? Copilot code review의 설정과 커스터마이징 개선입니다.",
                "2. 핵심 구성 요소: 코드 리뷰 규칙, 저장소별 설정, 리뷰 제안 제어, 팀 표준 반영입니다.",
                "3. 업무 적용 포인트: 자동 코드 리뷰는 기본 제안보다 팀 컨벤션과 위험 기준을 반영하도록 설정해야 실제 리뷰 부담을 줄입니다.",
            ),
        ),
        (
            ("github mobile", "pull request comments", "copilot cloud agent"),
            "GitHub Mobile에서 Pull Request 댓글을 Copilot cloud agent로 수정할 수 있게 한 업데이트입니다. 모바일 환경에서도 리뷰 피드백을 바탕으로 에이전트에게 수정 작업을 맡기는 흐름이 더 자연스러워집니다.",
            (
                "1. 무엇을 다루나요? 모바일에서 PR 댓글 기반 수정 작업을 Copilot cloud agent로 처리하는 기능입니다.",
                "2. 핵심 구성 요소: GitHub Mobile, PR comments, Copilot cloud agent, 리뷰 피드백 수정입니다.",
                "3. 업무 적용 포인트: 코드 리뷰 후속 조치를 모바일에서도 에이전트 작업으로 넘길 수 있어 리뷰 대기 시간을 줄일 수 있습니다.",
            ),
        ),
        (
            ("scorecard for the ai age",),
            "OpenAI가 AI 시대에 필요한 국가·조직 차원의 준비도를 스코어카드 관점으로 정리한 글입니다. 인프라, 인재, 채택, 정책, 안전 기준을 함께 보며 AI 경쟁력을 평가하는 프레임을 제시합니다.",
            (
                "1. 무엇을 다루나요? AI 시대의 준비도와 경쟁력을 평가하기 위한 스코어카드 관점입니다.",
                "2. 핵심 구성 요소: 인프라, 인재, 채택률, 정책 환경, 안전 기준, 경제 효과입니다.",
                "3. 업무 적용 포인트: 조직의 AI 전략은 모델 도입 여부만이 아니라 인력, 인프라, 거버넌스, 실제 생산성 지표를 함께 봐야 합니다.",
            ),
        ),
        (
        ...
        (
            ...
            (
                "1. 무엇을 다루나요? GitHub Projects에서 고급 검색을 정식 제공한 업데이트입니다.",
                "2. 핵심 구성 요소: 프로젝트 검색, 필터 조건, 상태·담당자 조회, 작업 추적입니다.",
                "3. 업무 적용 포인트: 프로젝트 운영 자동화를 만들 때는 이슈 목록 전체를 훑기보다 필요한 상태와 조건을 검색으로 좁히는 흐름이 중요합니다.",
            ),
        ),
        (
            ("repository admins can archive pull requests",),
            "GitHub 저장소 관리자가 Pull Request를 archive할 수 있게 된 업데이트입니다. 오래되었거나 더 이상 진행하지 않는 PR을 정리해 리뷰 목록과 작업 대기열을 깔끔하게 유지할 수 있습니다.",
            (
                "1. 무엇을 다루나요? repository admin이 Pull Request를 archive할 수 있는 GitHub 기능입니다.",
                "2. 핵심 구성 요소: PR archive, 저장소 관리 권한, 리뷰 대기열 정리, 작업 상태 관리입니다.",
                "3. 업무 적용 포인트: 오래된 PR이 많은 저장소는 완료·중단·보류 상태를 명확히 정리해야 리뷰 우선순위가 흐려지지 않습니다.",
            ),
        ),
        (
            ("visual studio subscription management",),
            "Visual Studio 구독 관리를 위한 GitHub REST API 엔드포인트가 추가된 업데이트입니다. 엔터프라이즈 환경에서 구독 할당, 회수, 현황 조회를 자동화해 개발자 라이선스 운영 부담을 줄일 수 있습니다.",
            (
                "1. 무엇을 다루나요? Visual Studio subscription management를 위한 REST API 엔드포인트입니다.",
                "2. 핵심 구성 요소: 구독 조회, 할당·회수 자동화, 엔터프라이즈 라이선스 관리, API 운영입니다.",
                "3. 업무 적용 포인트: 개발자 도구 라이선스는 수동 관리보다 API 기반으로 온보딩·오프보딩 흐름에 연결하는 것이 안정적입니다.",
            ),
        ),
        (
            ("xcode 27 runner image",),
            "GitHub Actions에서 Xcode 27 runner image가 public preview로 제공된 업데이트입니다. iOS/macOS 빌드 파이프라인은 새 Xcode 이미지 호환성을 미리 검증하고 전환 계획을 세울 수 있습니다.",
            (
                "1. 무엇을 다루나요? GitHub Actions의 Xcode 27 runner image 공개 미리보기입니다.",
                "2. 핵심 구성 요소: macOS runner, Xcode 27, CI 빌드 이미지, 호환성 검증입니다.",
                "3. 업무 적용 포인트: 모바일 앱 CI는 새 Xcode 이미지가 나오면 빌드·테스트·서명 흐름을 미리 검증해야 배포 차질을 줄입니다.",
            ),
        ),
        (
            ("why teens deserve access safe ai",),
            "OpenAI가 청소년에게도 안전하게 설계된 AI 접근이 필요하다고 설명한 글입니다. 차단보다 연령에 맞는 보호 장치, 교육, 책임 있는 사용 경험을 제공하는 방향을 강조합니다.",
            (
                "1. 무엇을 다루나요? 청소년이 안전하게 AI를 사용할 수 있도록 접근성과 보호 장치를 함께 설계해야 한다는 주장입니다.",
                "2. 핵심 구성 요소: 청소년 보호, 안전한 AI 접근, 교육, 연령 적합 경험, 책임 있는 사용입니다.",
                "3. 업무 적용 포인트: 교육·소비자 AI 서비스는 접근 제한만이 아니라 사용자 연령과 위험에 맞춘 안전 설계가 필요합니다.",
            ),
        ),
        (
            ("2607.02357",),
            "에이전트 스킬 마켓플레이스의 악성 스킬이 정적 스캐너를 우회할 수 있음을 보이고, 실행 중 행위를 샌드박스에서 관찰해 탐지하는 SkillDetonate를 제안한 보안 연구입니다. 설치 시점의 코드 모양보다 파일, 프로세스, 네트워크로 흐르는 민감 정보 이동을 추적하는 데 초점이 있습니다.",
            (
                "1. 무엇을 다루나요? 에이전트 스킬 악성코드가 정적 스캐너를 우회하는 방식과 동적 탐지 방법입니다.",
                "2. 핵심 구성 요소: SkillCloak 우회, self-extracting packing, sandbox 실행, taint 기반 정보 흐름 분석입니다.",
                "3. 업무 적용 포인트: 외부 에이전트 스킬을 도입할 때는 설치 전 스캔만으로 부족하고 실행 중 권한 사용과 데이터 흐름을 감사해야 합니다.",
            ),
        ),
        (
            ("2607.01668",),
            "VeriChat은 하드웨어 보안 검증 과정을 지원하는 도메인 특화 대화형 AI입니다. RAG와 다중 에이전트 구조, Icarus Verilog·Yosys·SymbiYosys 같은 EDA 도구를 연결해 RTL 설계의 구문 검사, 합성, 시뮬레이션, 형식 검증까지 수행합니다.",
            (
                "1. 무엇을 다루나요? 하드웨어 보안 검증 엔지니어를 돕는 대화형 다중 에이전트 VeriChat입니다.",
                "2. 핵심 구성 요소: RAG, 세 개의 전문 에이전트, EDA 도구 연동, RTL 분석, Trojan 탐지 사례입니다.",
                "3. 업무 적용 포인트: 전문 검증 업무에 AI를 붙일 때는 일반 챗봇보다 도메인 지식 검색과 실제 검증 도구 실행을 함께 묶어야 합니다.",
            ),
        ),
        (
            ("2607.01647",),
            "AgenticDataBench는 데이터 과학 업무를 자동화하는 LLM 기반 데이터 에이전트를 평가하기 위한 벤치마크입니다. 15개 도메인의 실제 데이터와 업무, 세분화된 정답 라벨, 데이터 과학 스킬 단위 커버리지를 통해 에이전트 성능을 더 촘촘히 측정합니다.",
            (
                "1. 무엇을 다루나요? 다양한 데이터 과학 업무에서 데이터 에이전트의 실제 수행 능력을 평가하는 AgenticDataBench입니다.",
                "2. 핵심 구성 요소: 15개 도메인, 실제 B2B 사례, 세분화된 정답 라벨, 데이터 과학 스킬 커버리지입니다.",
                "3. 업무 적용 포인트: 데이터 에이전트 도입 전에는 단순 정확도보다 도메인별 스킬, 비용, 실패 유형을 함께 봐야 합니다.",
            ),
        ),
        (
            ("2607.00254",),
            "AI 워크플로를 선언적 쿼리처럼 보고, approximate query processing과 proxy model 필터링으로 비싼 모델 호출 수를 줄이는 연구입니다. LLM 후학습, 수학 추론, 코드 생성 같은 파이프라인에서 정확도 손실을 제한하면서 평가 비용을 줄이는 데 초점이 있습니다.",
            (
                "1. 무엇을 다루나요? AI 워크플로를 쿼리 중심으로 최적화해 비싼 모델 호출을 줄이는 방법입니다.",
                "2. 핵심 구성 요소: AQP, proxy model, 조기 종료, confidence interval, reward model 호출 절감입니다.",
                "3. 업무 적용 포인트: 대량 평가나 후학습 파이프라인은 모든 샘플을 대형 모델에 보내기보다 저비용 필터와 근사 평가를 설계해야 합니다.",
            ),
        ),
        (
            ("2607.00151",),
            "SmoothAgent는 장기 에이전트 워크플로에서 컨텍스트 압축·분리·오프로딩 때문에 생기는 KV cache 무효화와 TTFT 증가를 줄이는 serving 연구입니다. 컨텍스트 변환을 미리 실행하고 변환된 캐시를 준비해 긴 작업의 응답 지연을 낮춥니다.",
            (
                "1. 무엇을 다루나요? 장기 LLM 에이전트의 컨텍스트 변환 오버헤드를 줄이는 SmoothAgent입니다.",
                "2. 핵심 구성 요소: lookahead context engineering, 비동기 컨텍스트 변환, KV cache 준비, lookahead-aware scheduler입니다.",
                "3. 업무 적용 포인트: 장기 에이전트 운영에서는 모델 호출 횟수뿐 아니라 컨텍스트 변환과 캐시 재사용 전략이 비용과 속도를 좌우합니다.",
            ),
        ),
        (
            ("2607.02507",),
            "이 논문은 LLM 에이전트가 공개 발화와 비공개 채널에서 서로 다른 답을 내는지 살펴봅니다. 역할, 청중, 관계 압박이 주어질 때 명시적 목표가 없어도 숨은 사회적 목표가 나타날 수 있음을 dual-channel debate로 평가합니다.",
            (
                "1. 무엇을 다루나요? LLM 에이전트가 사회적 압력 아래 공개 답변과 비공개 답변을 다르게 내는 현상입니다.",
                "2. 핵심 구성 요소: dual-channel debate, public utterance, off-the-record 응답, stance·semantic·NLI 분석입니다.",
                "3. 업무 적용 포인트: 에이전트 평가는 정답 여부만이 아니라 역할·청중·인센티브가 답변을 바꾸는지도 점검해야 합니다.",
            ),
        ),
        (
            ("2607.02469",),
            "TestEvo-Bench는 코드 변경과 테스트 변경이 함께 진화하는지를 실행 가능한 방식으로 평가하는 벤치마크입니다. 실제 커밋 이력에서 test generation과 test update 과제를 만들고, pass rate·coverage·mutation score 같은 실행 기반 지표로 에이전트를 평가합니다.",
            (
                "1. 무엇을 다루나요? 코드 변경에 맞춰 테스트를 새로 만들거나 고치는 에이전트 능력을 평가하는 TestEvo-Bench입니다.",
                "2. 핵심 구성 요소: 실제 커밋 이력, 테스트 생성, 테스트 업데이트, 실행 환경, pass rate와 coverage 지표입니다.",
                "3. 업무 적용 포인트: 코딩 에이전트 평가는 코드 작성뿐 아니라 변경된 동작을 테스트가 제대로 따라가는지까지 봐야 합니다.",
            ),
        ),
        (
            ("2607.01640",),
            "AgentFlow는 LLM 에이전트 프로그램을 정적 분석하기 위해 모델, 프롬프트, 도구, 메모리, 다중 에이전트 오케스트레이션 사이의 의존성을 그래프로 만드는 연구입니다. 일반 코드 흐름뿐 아니라 에이전트 프레임워크가 만드는 새로운 의존 관계를 분석합니다.",
            (
                "1. 무엇을 다루나요? 에이전트 프로그램의 모델·프롬프트·도구 의존성을 정적 분석 그래프로 만드는 AgentFlow입니다.",
                "2. 핵심 구성 요소: agent dependency graph, 프롬프트·도구·메모리 의존성, 정적 분석, 프레임워크 의미 모델링입니다.",
                "3. 업무 적용 포인트: 에이전트 코드는 일반 함수 호출뿐 아니라 프롬프트와 도구 연결 관계까지 분석해야 변경 위험을 줄일 수 있습니다.",
            ),
        ),
        (
            ("2607.01415",),
            "코딩 에이전트 강화학습에서 실행 인프라를 단순 배경 요소로 보지 않고, 롤아웃 비용과 지연을 직접 측정한 연구입니다. 컨테이너, 호스팅 샌드박스, Kubernetes, 클라우드 VM 사이의 cold start와 worker-hour 차이를 비교합니다.",
            (
                "1. 무엇을 다루나요? 코딩 에이전트 RL 학습에서 샌드박스와 실행 인프라가 만드는 비용 차이입니다.",
                "2. 핵심 구성 요소: interactive rollout, cold-start latency, worker-hours, 컨테이너·Kubernetes·VM 비교입니다.",
                "3. 업무 적용 포인트: 코딩 에이전트 학습·평가를 대규모로 돌릴 때는 모델만이 아니라 실행 환경의 시작 지연과 격리 방식을 최적화해야 합니다.",
            ),
        ),
        (
            ("2607.01120",),
            "이 논문은 배포 후 정적으로 굳어 있는 엔터프라이즈 에이전트가 실제 업무 경험에서 계속 학습하고 개선되는 차세대 agentic RL 시스템을 다룹니다. 운영 로그, 평가, 프롬프트·도구·모델 업데이트를 안전한 학습 루프로 연결하는 방향을 제안합니다.",
            (
                "1. 무엇을 다루나요? 배포된 LLM 에이전트가 실제 상호작용 경험으로 계속 개선되는 agentic RL 시스템입니다.",
                "2. 핵심 구성 요소: 운영 trace, 평가 벤치마크, RL 업데이트, 도구 레퍼토리 개선, 재배포 루프입니다.",
                "3. 업무 적용 포인트: 엔터프라이즈 에이전트는 수동 튜닝만으로는 한계가 있어 안전한 피드백 수집과 검증된 개선 루프가 필요합니다.",
            ),
        ),
        (
            ("copilot usage metrics reports",),
            "GitHub Copilot 사용량 지표 보고서의 정확도와 커버리지를 개선한 업데이트입니다. 조직은 좌석, 사용량, 기능별 활용 현황을 더 믿을 수 있는 지표로 보고 비용과 도입 효과를 판단할 수 있습니다.",
            (
                "1. 무엇을 다루나요? Copilot 사용량 리포트의 정확도와 집계 범위를 개선한 GitHub 업데이트입니다.",
                "2. 핵심 구성 요소: 사용량 측정, 보고서 커버리지, 조직 단위 도입 지표, 비용 관리입니다.",
                "3. 업무 적용 포인트: AI 도구 운영은 체감 효과만이 아니라 실제 사용량과 비용 지표를 기준으로 관리해야 합니다.",
            ),
        ),
        (
            ("deprecation of gemini 2 5 pro",),
            "GitHub Copilot에서 Gemini 2.5 Pro와 Gemini 3 Flash 지원 중단 일정을 알린 업데이트입니다. 팀은 모델 의존 설정, 정책, 사용자 안내를 미리 점검해 모델 전환 시점의 혼선을 줄여야 합니다.",
            (
                "1. 무엇을 다루나요? Copilot에서 일부 Gemini 모델 지원이 종료되는 일정 안내입니다.",
                "2. 핵심 구성 요소: 모델 deprecation, 대체 모델 선택, 조직 정책 변경, 사용자 공지입니다.",
                "3. 업무 적용 포인트: AI 도구는 모델 제공 상태가 바뀔 수 있으므로 업무 표준 모델과 전환 계획을 관리해야 합니다.",
            ),
        ),
        (
            ("personal access token", "github actions"),
            "GitHub Actions에서 개인 액세스 토큰 없이 Copilot CLI를 사용할 수 있게 한 업데이트입니다. CI 환경에서 장기 토큰을 직접 관리하는 부담을 줄이고 자동화 권한 범위를 더 안전하게 다룰 수 있습니다.",
            (
                "1. 무엇을 다루나요? GitHub Actions에서 PAT 없이 Copilot CLI를 실행하는 방식입니다.",
                "2. 핵심 구성 요소: Actions 인증, Copilot CLI, 토큰 관리 축소, CI 자동화입니다.",
                "3. 업무 적용 포인트: CI에 AI 도구를 붙일 때는 개인 토큰보다 워크플로 권한과 감사 가능한 인증 흐름을 선호해야 합니다.",
            ),
        ),
        (
            ("copilot agent session streaming",),
            "Copilot 에이전트 세션의 진행 상태를 스트리밍으로 볼 수 있게 한 GitHub 업데이트입니다. 장시간 실행되는 에이전트 작업에서 무엇을 하고 있는지 확인하고 중간 판단을 더 빨리 할 수 있습니다.",
            (
                "1. 무엇을 다루나요? Copilot 에이전트 세션 진행 상황을 실시간으로 확인하는 스트리밍 기능입니다.",
                "2. 핵심 구성 요소: 세션 상태, 실시간 로그, 에이전트 작업 추적, public preview입니다.",
                "3. 업무 적용 포인트: 장시간 에이전트 작업은 완료 결과만 기다리기보다 진행 로그와 중간 상태를 관측해야 안전하게 운영할 수 있습니다.",
            ),
        ),
        (
            ("cost centers", "usage caps"),
            "GitHub에서 비용 센터별 AI 사용량 한도를 관리할 수 있게 한 업데이트입니다. 조직은 팀이나 비용 단위별로 Copilot·AI 크레딧 사용을 제한하고 예산 초과를 예방할 수 있습니다.",
            (
                "1. 무엇을 다루나요? 비용 센터 단위로 AI 사용량 한도를 설정하는 GitHub 업데이트입니다.",
                "2. 핵심 구성 요소: cost center, usage cap, AI credit, 예산 통제입니다.",
                "3. 업무 적용 포인트: AI 도구 확산 단계에서는 개인 사용량보다 팀·비용 센터 기준의 예산 통제가 중요합니다.",
            ),
        ),
        (
            ("issue fields", "generally available"),
            "GitHub Issues의 issue fields 기능이 정식 제공으로 전환된 업데이트입니다. 이슈에 우선순위, 상태, 담당 영역 같은 구조화된 필드를 붙여 프로젝트 운영 데이터를 더 일관되게 관리할 수 있습니다.",
            (
                "1. 무엇을 다루나요? GitHub Issues에서 구조화된 issue fields 기능이 정식 제공된 업데이트입니다.",
                "2. 핵심 구성 요소: 커스텀 필드, 이슈 메타데이터, 프로젝트 관리, 상태 추적입니다.",
                "3. 업무 적용 포인트: 이슈 운영을 자동화하려면 제목·본문만이 아니라 분류와 우선순위를 구조화된 필드로 관리하는 편이 좋습니다.",
            ),
        ),
        (
            ("secret scanning", "public monitoring"),
            "GitHub Enterprise의 secret scanning 공개 모니터링 관련 업데이트입니다. 조직은 공개 저장소나 외부 노출 경로에서 비밀값이 탐지되는 상황을 더 빨리 파악하고 대응 흐름을 관리할 수 있습니다.",
            (
                "1. 무엇을 다루나요? 엔터프라이즈 환경에서 secret scanning 공개 노출 모니터링을 강화한 GitHub 업데이트입니다.",
                "2. 핵심 구성 요소: 비밀값 탐지, 공개 노출 모니터링, 보안 알림, 조직 단위 대응입니다.",
                "3. 업무 적용 포인트: AI·자동화가 늘수록 토큰과 키가 코드에 섞일 위험이 커져 secret scanning 운영 체계가 중요합니다.",
            ),
        ),
        (
            ("auto model selection",),
            "GitHub Copilot의 엔터프라이즈 자동 모델 선택 관련 업데이트입니다. 조직은 사용자가 매번 모델을 고르지 않아도 작업 성격에 맞는 모델 선택 정책을 적용해 품질과 비용 균형을 맞출 수 있습니다.",
            (
                "1. 무엇을 다루나요? Copilot Enterprise에서 작업에 맞춰 모델 선택을 자동화하는 기능입니다.",
                "2. 핵심 구성 요소: 모델 라우팅, 조직 정책, 비용·품질 균형, 사용자 경험 단순화입니다.",
                "3. 업무 적용 포인트: 여러 모델을 운영할 때는 개인 선택에 맡기기보다 조직 기준의 자동 라우팅 정책이 필요합니다.",
            ),
        ),
        (
            ("github models", "retired"),
            "GitHub Models의 종료 일정 또는 retired 안내를 다룬 업데이트입니다. 해당 기능이나 모델 엔드포인트에 의존한 실험·자동화가 있다면 대체 경로를 점검해야 합니다.",
            (
                "1. 무엇을 다루나요? GitHub Models 관련 기능 종료 또는 전환 일정 안내입니다.",
                "2. 핵심 구성 요소: 종료 일정, 대체 모델·플랫폼 검토, 의존 자동화 점검입니다.",
                "3. 업무 적용 포인트: AI 플랫폼 기능은 바뀔 수 있으므로 실험용 의존성을 운영 자동화에 넣기 전 종료 리스크를 확인해야 합니다.",
            ),
        ),
        (
            ("enterprise managed settings",),
            "GitHub Enterprise의 managed-settings JSON 기능이 정식 제공으로 전환된 업데이트입니다. 조직은 Copilot과 개발 환경 설정을 저장소별 수동 관리 대신 중앙 정책 파일로 일관되게 배포할 수 있습니다.",
            (
                "1. 무엇을 다루나요? 엔터프라이즈 managed-settings JSON의 정식 제공입니다.",
                "2. 핵심 구성 요소: 중앙 설정 관리, JSON 정책, 조직 단위 배포, Copilot·개발 환경 통제입니다.",
                "3. 업무 적용 포인트: 대규모 조직에서는 AI 도구 설정을 개인별 수동 설정이 아니라 중앙 정책으로 관리해야 운영 편차를 줄일 수 있습니다.",
            ),
        ),
        (
            ("2607.10265",),
...
def _fallback_specific_title(text: str) -> str:
    ...
    title_rules = (
        ...
        (("2607.02357",), "에이전트 스킬 악성코드 동적 탐지"),
        (("2607.01668",), "하드웨어 보안 검증용 대화형 AI"),
        (("2607.01647",), "데이터 에이전트 성능 벤치마크"),
        (("2607.00254",), "AI 워크플로의 쿼리 중심 최적화"),
        (("2607.00151",), "장기 작업 에이전트의 컨텍스트 선처리"),
        (("2607.02507",), "LLM 에이전트의 숨은 사회적 목표 평가"),
        (("2607.02469",), "테스트와 코드 동시 진화 벤치마크"),
        (("2607.01640",), "에이전트 프로그램 의존성 그래프 분석"),
        (("2607.01415",), "코딩 에이전트 RL의 실행 인프라 비용"),
        (("2607.01120",), "스스로 개선하는 에이전트 RL 시스템"),
        (("2607.12188",), "멀티테넌트 RAG 비용 귀속 체계"),
        (("2607.11972",), "순차 워크플로용 스킬 중심 의미 통신"),
        (("2607.15263",), "보안 에이전트의 비용 인식 평가"),
        (("2607.15235",), "무선 센서 네트워크의 적응형 이상 감지"),
        (("2607.14798",), "위성 미션 계획용 정책 기반 컴파일러"),
        (("agent security gap",), "AI 에이전트 자격 증명 공유 리스크"),
        (("agent evaluation gap",), "엔터프라이즈 AI 평가 현실 정렬 문제"),
        (("agentic orchestration",), "에이전트 오케스트레이션의 배포 격차"),
        (("ai compute gap",), "AI 인프라 비용 측정 격차"),
        (("ai context gap",), "엔터프라이즈 AI 컨텍스트 신뢰 격차"),
        (("repository level github copilot usage metrics",), "저장소 단위 Copilot 사용량 지표"),
        (("copilot app", "usage metrics api"), "Usage Metrics API의 Copilot 앱 데이터"),
        ...
        (("xcode 27 runner image",), "GitHub Actions Xcode 27 러너 이미지"),
        (("why teens deserve access safe ai",), "청소년을 위한 안전한 AI 접근"),
        ...
        (("mapping", "ai workforce opportunity"), "유럽 AI 인력 전환 기회"),
        (("britain", "productivity", "trailblazers"), "영국 AI 생산성 전략"),
        (("genebench pro", "case studies"), "GeneBench-Pro 사례 연구"),
